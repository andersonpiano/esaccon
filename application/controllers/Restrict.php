...
class Restrict extends CI_Controller{
	...
	public function ajax_save_member() {
		...
		if (!empty($json["error_list"])) {
			...
			if (!empty($data["member_photo"])) {
				...
				$old_path = getcwd() . "/tmp/" . $file_name;
				$new_path = getcwd() . "/public/images/setor/" . $file_name;
				rename($old_path, $new_path);

				$data["member_photo"] = "/public/images/setor/" . $file_name;
			...
			if (empty($data["member_id"])) {
				$this->setor_model->insert($data);
			} else {
				$member_id = $data["member_id"];
				unset($data["member_id"]);
				$this->setor_model->update($member_id, $data);
			}
		}
	...
	public function ajax_get_member_data() {
		...
		$data = $this->setor_model->get_data($member_id)->result_array()[0];
	...
	public function ajax_delete_member_data() {
		...
		$this->load->model("setor_model");
		$member_id = $this->input->post("member_id");
		$this->setor_model->delete($member_id);
	...
	public function ajax_list_member() {
		...
		$this->load->model("setor_model");
		$setor = $this->setor_model->get_datatable();

		$data = array();
		foreach ($setor as $member) {
